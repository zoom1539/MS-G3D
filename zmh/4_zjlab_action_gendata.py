...
class Feeder_zjlab_action(Dataset):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='zjlab_action-skeleton Data Converter.')
    parser.add_argument(
        '--data_path', default='/data1/zhumh/zjlab_actions/train_data')
    parser.add_argument(
        '--out_folder', default='/data1/zhumh/zjlab_actions/train_data')
    arg = parser.parse_args()

    part = ['val', 'train']
    for p in part:
        print('zjlab_action ', p)
        if not os.path.exists(arg.out_folder):
            os.makedirs(arg.out_folder)
        data_path = '{}/zjlab_action_{}'.format(arg.data_path, p)
        label_path = '{}/zjlab_action_{}_label.json'.format(arg.data_path, p)
